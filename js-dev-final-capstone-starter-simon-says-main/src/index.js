...
function setText(element, text) {
  ...
  if (!el) return { textContent: undefined };
...
function activatePad(color) {
  ...
  try {
    pad.sound.currentTime = 0;
    const maybePromise = pad.sound.play();
...
function activatePads(sequence) {
  sequence.forEach((color, i) => {
    setTimeout(() => activatePad(color), (i + 1) * 600);
  });
}
...
function playHumanTurn() {
  ...
  setText(statusSpan, `Player's turn... ${remaining} presses left`);
}
