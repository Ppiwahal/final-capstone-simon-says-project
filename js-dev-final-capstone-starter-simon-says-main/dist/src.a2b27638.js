...
function startButtonHandler() {
  ...
  // tests expect these returned (used only by tests)
  return {
...
function setText(element, text) {
  var el = typeof NodeList !== "undefined" && element instanceof NodeList ? element[0] : element;
  if (!el) return {
    textContent: undefined
  };
  el.textContent = text;
  // Return a serializable shape the test can read.
  return {
...
function activatePad(color) {
  ...
  // Required by tests; also guard for headless environments.
  try {
    pad.sound.currentTime = 0;
    var maybePromise = pad.sound.play();
...
function playComputerTurn() {
  ...
  // add a random color
  var randomColor = getRandomItem(pads).color;
...
function playHumanTurn() {
  ...
  var remaining = computerSequence.length - playerSequence.length;
  setText(statusSpan, "Player's turn... ".concat(remaining, " presses left"));
}
...
function checkPress(color) {
  ...
  setText(statusSpan, "Player's turn... ".concat(remainingPresses, " presses left"));
  if (computerSequence[index] !== color) {
    resetGame("Oops! Wrong pad. Game over.");
    return;
  }
...
function resetGame(text) {
  ...
  // UI reset (required by tests) — uncommented per spec:
  alert(text);
